...
.factory('AccessToken', function($rootScope, $location, $sessionStorage, $interval){
    ...
    service.set = function(){
        ...
        if ( service.token === null && Object.keys($location.search()).length > 0 ) {
...
angular.module('oauth.endpoint', [])
.factory('Endpoint', function(AccessToken, $location) {
...
.factory('Profile', function($http, AccessToken, $rootScope) {
  var service = {};
  var profile;
...
.factory('ExpiredInterceptor', function ($rootScope, $q, $sessionStorage) {
...
.directive('oauth', function(AccessToken, Endpoint, Profile, $location, $rootScope, $compile, $http, $templateCache) {
  ...
  definition.link = function postLink(scope, element, attrs) {
    ...
    var initAttributes = function() {
      ...
      scope.tokenPath     = scope.tokenPath     || '/oauth/access_token';
      scope.template      = scope.template      || 'app/login/oauth/default.html';
      scope.responseType  = scope.responseType  || 'token';
      scope.text          = scope.text          || 'Sign In';
      scope.iconPath      = scope.iconPath      || '';
