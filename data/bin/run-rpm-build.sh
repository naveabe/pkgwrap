...
if [ "$BUILD_TYPE" == "source" ]; then
    ...
    if [ "$BUILD_CMD" != "" ]; then
        ...
        su - $BUILD_USER -c "cd ~/rpmbuild/SOURCES/$PROJECT && ( find . -type f | sed s/^\.//g >> ~/rpmbuild/SPECS/$PROJECT.spec ) && cd -" || {
            ...
            fire_event_exit "build:failed" "$BUILD_USER/$PROJECT" 5
        }
  ...
  su - $BUILD_USER -c "cp -a $PROJECT_PATH $BUILD_HOME_DIR/rpmbuild/SOURCES/" || {
    echo "Could not copy binary files!"
    exit 3;
  }
fi

fire_build_event "package:rpm:started" "$BUILD_USER/$PROJECT";
#
# Build rpm from spec
...
su - $BUILD_USER -c "QA_RPATHS=$[ 0x0001|0x0010 ] rpmbuild -ba ~/rpmbuild/SPECS/$PROJECT.spec" || {
    fire_event_exit "package:rpm:failed" "$BUILD_USER/$PROJECT" 6;
}

fire_build_event "package:rpm:succeeded" "$BUILD_USER/$PROJECT"
